...
- 健康检查必须区分：不可达、已确认 DSH、可达但身份未知、需要认证（身份尚未确认）、外部重定向和无效 URI；不得把任意 2xx 或 401/403 服务当作 DSH。外部认证只接收用户主动提供的当前 loopback 源固定格式链接，通过独立 Cookie 交换及 HTML 双身份校验后才能导航；输入不进入配置或日志。
