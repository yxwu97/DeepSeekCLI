...
| `DSH-E205` | 端口已有服务，但未能确认是 DSH | 是（检查服务地址后） |
| `DSH-E228` | 本机服务需要认证，DSH 身份尚未确认 | 是（提供当前源的 DSH 认证链接后） |
| `DSH-E229` | 认证链接格式或地址不符 | 是（提供完整匹配的链接后） |
...
### 37.2 外部实例认证接入安全评审（2026-09-13，0.13.0）

- 401/403 独立分类为 `AuthenticationRequired`，使用 `DSH-E228`；该结果仍不能确认 DSH 身份，也不能加载 WebView2 或启动进程争抢端口。初始化和启动预检复用合法的未知身份失败迁移；运行监控失效后清除外部会话。
- 用户主动在专用遮蔽输入框粘贴 DSH 终端中的完整认证 URL，不扫描其他进程内存、输出文件或浏览器数据。只接受当前配置的 loopback scheme/host/port、根路径和固定 43 字符 base64url token；localhost/IPv6 仅可沿用同 scheme/端口的 IPv4 官方源规范化。拒绝远程、相邻恶意域名、用户信息、其他端口、附加参数、片段和编码变体。非法链接使用 `DSH-E229`，不回显原文。
- `IExternalDshConnector` 与生命周期协调器共享同一 DI 实例及操作门。认证不调用安装、命令解析或进程启动；仅在停止/失败状态接入，取消保留 generation 校验并清理会话。仍须先执行同源 303、独立临时 Cookie 交换及原有 HTML 双身份校验，通过后合法迁移到 `RunningExternal`。
- Code WebView2 沿用既有认证交换与同源导航限制，不复制 HTTP 或其他浏览器 Cookie。Chat profile、origin、下载及权限策略不变。外部认证 URL 仅保留内存，输入框提交或卸载后清空，失败、取消、成功切换地址、健康丢失和宿主退出时清除。
- 回归覆盖未认证响应不冒充 DSH、错误 HTML、认证前后跨源零请求、恶意 URL、失败清理和取消，以及接入后的命令可用性。当前流程要求用户提供一次有效认证链接，不把“端口有服务”当成已授权会话。
